...
triang <- function(x1, y1, alpha1, x2, y2, alpha2) {
  ...
  if ((abs(alpha1 - alpha2) %% 180) <= 0.001) {
    return(c(NA, NA))
  }
  ...
  l <- tryCatch(
    ...
    error = function(e) {
      write(paste0("Could not biangulate for positions (X=", x1, ", Y=", y1, ", a=", alpha1, ") and (X=", x2, ", Y=", y2, ", a=", alpha2, ")"), stderr())
...
speed_between_triangulations <- function(timestamp, longitude, latitude) {
  tmp <- data.frame(
    timediff = c(0, as.numeric(diff(timestamp))),
