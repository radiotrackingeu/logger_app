...
smoothed_curves <- reactive({
  ...
  for(i in unique(data$receiver)){
    ...
    min_time<-min(tmp$timestamp)
    max_time<-max(tmp$timestamp)
    smoothed<-data.frame(smoothed_data=predict(
      smooth.spline(tmp$timestamp,tmp$max_signal,spar=input$spar_in),
      as.numeric(seq(min_time,max_time,1)))$y,
      timestamp=seq(min_time,max_time,1),
      receiver=i)
...
plot_time_signal <- function(data, multifilter){
  p<-ggplot() +
    geom_point(data=data,aes(data[[input$select_x]], data[[input$select_y]], color=data[[input$select_col]]), size=I(0.8)) +
    labs(x="Time", y = "Signal Strength") +
    scale_x_datetime(labels = function(x) format(x, "%d-%m \n %H:%M:%S"))+
    facet_wrap(input$select_facet)+
    geom_point(data=smoothed_curves(),aes(x=smoothed_curves()$timestamp,y=smoothed_curves()$smoothed_data,col=smoothed_curves()$receiver))
  if(multifilter){
    p + facet_wrap(input$select_facet)
  }
  else{
    p
  }
